"""Training-only J1c execution surface and outcome-free readiness sealer.
...
VERSION = "j1c_training_execution_surface_v1"
REPO_ROOT = Path(__file__).resolve().parents[1]
RUNS_ROOT = REPO_ROOT / "threes_rl" / "runs"
CHARTER_PATH = (
    REPO_ROOT
    / "threes_rl"
    / "J1C_TRAINING_EXECUTION_ORCHESTRATION_REPAIR_CHARTER.md"
)
RUNNER_PATH = REPO_ROOT / "threes_rl" / "j1c_training_execution_surface.py"
TEST_PATH = REPO_ROOT / "tests" / "test_rl_j1c_training_execution_surface.py"
READINESS_DIR = (
    RUNS_ROOT
    / "forensics"
...
FUTURE_EXECUTION_ROOT = (
    RUNS_ROOT / "forensics" / "j1c_execution_v1"
)
J1B_PREFLIGHT_DIR = (
...
EXPECTED_CANONICAL_ROWS_SHA256 = (
    "4838b2167f38e982b5edf0f6b4b500ac7899ef3e7c0545fa1b8ba880c085e99d"
)
EXPECTED_ROOT_COMMITMENT_PAYLOAD_SHA256 = (
    "9cccd58e5f88e3bae9351a4dc07a10936810f097a66b9c7cedc9fe4dae26e24e"
)
EXPECTED_ROOT_SET_SHA256 = (
    "ad6192fa5126614413cb90eeb35f264675113b63da0d8e14d2102e2da6a7ed51"
)
...
TRAIN_ROOTS = 16_384
STREAM_RANGES = {
    "logical_stream_id": (213_000_032_768, 213_000_049_151),
    "deck_stream_id": (214_000_032_768, 214_000_049_151),
    "slot_stream_id": (215_000_032_768, 215_000_049_151),
...
TEST_EVIDENCE_NAME = "J1C_TRAINING_EXECUTION_TEST_EVIDENCE.json"
SOURCE_MANIFEST_NAME = "J1C_PROSPECTIVE_TRAINING_MANIFEST.json"
STREAM_AUTHORITY_NAME = "J1C_PROTECTED_STREAM_AUTHORITY.json"
SCHEMA_NAME = "J1C_TRAINING_EXECUTION_SCHEMA.json"
PROJECTION_NAME = "J1C_TRAINING_EXECUTION_PROJECTION.json"
INPUT_BINDINGS_NAME = "J1C_TRAINING_EXECUTION_INPUT_BINDINGS.json"
READINESS_LOCK_NAME = "J1C_TRAINING_EXECUTION_READINESS_LOCK.json"
READINESS_RESULT_NAME = "J1C_TRAINING_EXECUTION_READINESS_RESULT.json"
READY_DECISION = "READY_J1C_TRAINING_EXECUTION_SURFACE"
HOLD_DECISION = "HOLD_J1C_TRAINING_EXECUTION_SURFACE"
KILL_DECISION = "KILL_J1C_TRAINING_EXECUTION_SURFACE_INTEGRITY"


class J1cSurfaceError(RuntimeError):
    """Base error for the J1c training-only surface."""


class J1cSurfaceIntegrityError(J1cSurfaceError):
    """Immutable identity or scientific execution integrity failed."""


class J1cSurfaceOperationalHold(J1cSurfaceError):
...
def load_json(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise J1cSurfaceIntegrityError(
...
def bound_command(
    ...
    *,
    ...
    if action not in PUBLIC_COMMANDS:
        raise ValueError(f"Unsupported J1c command: {action}")
    return shlex.join(
        [
            "nice",
            "-n",
            ...
            ".venv/bin/python",
            "-m",
            "threes_rl.j1c_training_execution_surface",
            action,
            "--execution-root",
...
def j1c_root_commitment() -> dict[str, Any]:
    payload = {
        ...
        "j1c_charter_file_sha256": EXPECTED_CHARTER_SHA256,
...
def root_id_for_commitment(
    ...
    if not verify_payload_hash(commitment, "marker_payload_sha256"):
        raise J1cSurfaceIntegrityError(
            "J1c root commitment hash is invalid"
        )
    if (
        ...
        raise J1cSurfaceIntegrityError(
            "J1c root commitment/row mismatch"
        )
...
def prospective_training_manifest() -> dict[str, Any]:
    commitment = j1c_root_commitment()
...
def compact_stream_authority(
    ...
    j1b_manifest = load_json(
        Path(COMPACT_STREAM_AUTHORITIES["j1b_prospective"]["path"])
    )
    j1b_rows = list(j1b_manifest["rows"])
    rows = list(manifest["rows"])
    collisions = {
        field: sorted(
            set(int(row[field]) for row in rows)
            & set(int(row[field]) for row in j1b_rows)
        )
        for field in STREAM_RANGES
    }
    expected_spent = {
        "logical_stream_id": (213_000_000_000, 213_000_032_767),
        "deck_stream_id": (214_000_000_000, 214_000_032_767),
        "slot_stream_id": (215_000_000_000, 215_000_032_767),
        "candidate_policy_stream_id": (
            216_000_000_000,
            216_000_032_767,
        ),
    ...
    checks = {
        "four_compact_authorities_exact": len(verified) == 4,
        "j1b_declared_rows_exact": len(j1b_rows) == TRAIN_ROOTS,
        "j1b_declared_intervals_spent": all(
            min(int(row[field]) for row in j1b_rows) == spent[0] + TRAIN_ROOTS
            and max(int(row[field]) for row in j1b_rows) == spent[1]
            for field, spent in expected_spent.items()
        ),
        "j1c_starts_immediately_after_j1b": all(
            STREAM_RANGES[field][0] == spent[1] + 1
            for field, spent in expected_spent.items()
        ),
        "j1c_exact_16384_width": all(
            end - start + 1 == TRAIN_ROOTS
            for start, end in STREAM_RANGES.values()
        ),
        "zero_j1b_row_collisions": all(
            not values for values in collisions.values()
        ),
        "j1b_spent_despite_zero_consumption": True,
        "no_broad_payload_scan": True,
    }
    payload = {
        "version": f"{VERSION}_protected_stream_authority_v1",
        "method": (
            "exact compact J1/J1c manifests and interval authorities only"
        ),
        "authorities": verified,
        "spent_intervals_through_j1b": expected_spent,
        "fresh_j1c_intervals": dict(STREAM_RANGES),
...
def _validate_source_manifest(
    ...
    *,
    ...
    if not all(checks.values()):
        ...
        raise J1cSurfaceIntegrityError(
            "J1c source manifest validation failed: "
            + ", ".join(failed)
        )
    return {
        "row_count": len(rows),
...
def audit_authoritative_inputs(
    *,
    ...
    if (
        ...
        raise J1cSurfaceIntegrityError(
            "J1c source manifest payload changed"
        )
...
def load_ready_surface(readiness_dir: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise J1cSurfaceIntegrityError(
            "J1c execution-surface readiness lock is invalid"
        )
    if (
        not verify_payload_hash(
            result,
        ...
        raise J1cSurfaceIntegrityError(
            "J1c execution-surface readiness result is invalid"
        )
    ...
    if result.get("readiness_lock_identity") != lock_identity:
        raise J1cSurfaceIntegrityError(
            "J1c surface result changed its readiness lock"
        )
    mode = str(lock.get("execution_mode", ""))
    if mode not in {"scientific", "miniature_fixture"}:
        raise J1cSurfaceIntegrityError(
            "J1c readiness execution mode is invalid"
        )
    expected_sources = {
    ...
    if mode == "scientific":
        ...
        if lock.get("source_manifest_identity") != source_observed:
            raise J1cSurfaceIntegrityError(
                "J1c readiness changed source manifest"
            )
        if lock.get("stream_authority_identity") != authority_observed:
            raise J1cSurfaceIntegrityError(
                "J1c readiness changed stream authority"
            )
...
def _phase_ready_decision() -> str:
    return "READY_J1C_TRAINING_PHASE_LOCK"
...
def seal_training_phase_lock(
    *,
    ...
    if execution_root.exists():
        ...
        if present:
            raise FileExistsError(
                "J1c phase lock requires a fresh execution root: "
                + ", ".join(present)
            )
...
def load_training_phase_lock(
    *,
    ...
    if (
        ...
        raise J1cSurfaceIntegrityError(
            "J1c training phase lock is invalid"
        )
    if (
        not verify_payload_hash(
            lock_result,
        ...
        raise J1cSurfaceIntegrityError(
            "J1c training phase lock result is invalid"
        )
    lock_identity = immutable_json_identity(
        paths["lock"],
        payload_field="phase_lock_payload_sha256",
    ...
    if lock_result.get("phase_lock_identity") != lock_identity:
        raise J1cSurfaceIntegrityError(
            "J1c lock result changed phase lock identity"
        )
    if (
        ...
        raise J1cSurfaceIntegrityError(
            "J1c phase lock changed readiness identities"
        )
    for field, expected in {
        ...
        if lock.get(field) != expected:
            raise J1cSurfaceIntegrityError(
                f"J1c phase lock changed {field}"
            )
    expected_commands = {
        action: bound_command(
            action,
    ...
    if not all(checks.values()):
        failed = sorted(key for key, value in checks.items() if not value)
        raise J1cSurfaceIntegrityError(
            "J1c phase lock validation failed: " + ", ".join(failed)
        )
    return {
        "paths": paths,
        "readiness": readiness,
...
def real_operational_audit_marker_roundtrip(
    ...
    if operational.get("passes") is not True or not pre_write_is_tuple:
        raise J1cSurfaceOperationalHold(
    ...
    *,
    ...
    for name in (
        ...
        if paths[name].exists():
            raise FileExistsError(
                f"J1c open requires unused artifact: {paths[name]}"
            )
    if loaded["readiness"]["mode"] == "scientific":
        ...
        if operational.get("passes") is not True:
            raise J1cSurfaceOperationalHold(
                "J1c training open operational audit failed"
            )
    else:
    ...
    if state != expected:
        raise J1cSurfaceIntegrityError(
            "J1c open created work beyond the immutable marker"
        )
    return {
        "marker": written,
...
def load_open_training_contract(
    *,
    ...
    if (
        ...
        raise J1cSurfaceIntegrityError(
            "J1c open marker changed its immutable contract"
        )
    marker_identity = immutable_json_identity(
        paths["marker"],
        payload_field="activation_marker_payload_sha256",
    ...
    if require_manifest:
        ...
        if manifest != loaded["expected_manifest"]:
            raise J1cSurfaceIntegrityError(
                "J1c materialized manifest changed sealed rows"
            )
...
def materialize_training_manifest(
    *,
    ...
    if paths["manifest"].exists():
        raise FileExistsError(
            f"J1c manifest already exists: {paths['manifest']}"
        )
    for name in (
        "owner",
        "reservation",
        ...
        if paths[name].exists():
            raise J1cSurfaceIntegrityError(
                "J1c manifest materialization followed scientific work"
            )
    written = write_immutable_json(
        paths["manifest"],
        {
    ...
    if written != loaded["expected_manifest"]:
        raise J1cSurfaceIntegrityError(
            "J1c manifest materialization changed bytes"
        )
    return {
        "manifest": written,
        "manifest_identity": {
...
def _stream_inventory(
    ...
    for field in STREAM_RANGES:
        ...
        if len(values) != len(set(values)):
            raise J1cSurfaceIntegrityError(
                f"J1c manifest duplicated {field}"
            )
        roles[field] = {
            "count": len(values),
            "minimum": min(values),
    ...
    if not all(checks.values()):
        raise J1cSurfaceIntegrityError(
            "J1c stream inventory is invalid"
        )
    return {
        "row_count": len(rows),
...
def _seal_stream_reservation(
    *,
    ...
    payload = {
        ...
        "execute_command": loaded["commands"]["execute"],
        "decision": "RESERVED_J1C_TRAINING_STREAMS",
...
def _seal_stream_consumption(
    *,
    ...
    if (
        ...
        raise J1cSurfaceIntegrityError(
            "J1c stream reservation is invalid"
        )
    ...
    if paths["consumption"].exists():
        ...
        if not verify_payload_hash(
            ...
            raise J1cSurfaceIntegrityError(
                "Existing J1c stream consumption is invalid"
            )
        ...
        if not all(checks.values()):
            raise J1cSurfaceIntegrityError(
                "Recovered J1c stream consumption changed contract"
            )
        return {
    ...
    payload = {
        ...
        "execute_command": loaded["commands"]["execute"],
        "consumption_scope": "exact full immutable J1c training manifest",
        ...
        "decision": "OPENED_J1C_TRAINING_STREAM_CONSUMPTION",
        "passes": True,
    }
    written = write_immutable_json(
        paths["consumption"],
...
def _runtime_entrypoint(
    *,
    ...
    try:
        ...
        raise J1cSurfaceOperationalHold(str(error)) from error
...
def _seal_scientific_training_terminal(
    *,
    ...
    if engine_result.get("completed") is not True:
        raise J1cSurfaceIntegrityError(
            "J1c training engine did not complete"
        )
    ...
    if (
        ...
        raise J1cSurfaceIntegrityError(
            "J1c training terminal boundary is incomplete"
        )
...
def _seal_fixture_training_terminal(
    *,
    ...
    base.update(
        {
            "decision": "READY_J1C_MINIATURE_TRAINING_FIXTURE",
...
def _seal_failure_terminal(
    *,
    ...
    operational = isinstance(
        error,
        J1cSurfaceOperationalHold,
    ) or error.__class__.__name__ in {
        "J1ExecutionOperationalHold",
    }
    decision = (
        "HOLD_J1C_OPERATIONAL"
        if operational
        else "KILL_J1C_TRAINING_INTEGRITY"
    )
    base = _terminal_base(
        loaded=loaded,
        reservation_identity=reservation_identity,
...
def _load_terminal_result(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise J1cSurfaceIntegrityError(
            "Existing J1c terminal result is invalid"
        )
    return result
...
def _finalize_retention(
    *,
    ...
    if retention.get("retention_payload_sha256") != observed[
        ...
        raise J1cSurfaceIntegrityError(
            "J1c retention seal changed after write"
        )
    return {
        "result": result,
        "result_identity": immutable_json_identity(
...
def execute_training_from_artifacts(
    *,
    ...
    mode = loaded["readiness"]["mode"]
    fixture_env_names = (
        "J1C_FIXTURE_RUNTIME_FAILURE",
        "J1C_FIXTURE_FIRST_GUARD_FAILURE",
        "J1C_FIXTURE_PRE_ENGINE_INTERRUPT",
        "J1C_FIXTURE_INTERRUPT_AFTER_BOUNDARY",
    )
    ...
    if (
        ...
        and os.environ.get("J1C_FIXTURE_RUNTIME_FAILURE") == "1"
    ):
        raise J1cSurfaceOperationalHold(
    ...
    def after_guard(
        ...
        pre_engine_interrupt = (
            os.environ.get("J1C_FIXTURE_PRE_ENGINE_INTERRUPT")
            if mode == "miniature_fixture"
            else None
        )
    ...
    fixture_guard_failure = (
        mode == "miniature_fixture"
        and (
            os.environ.get("J1C_FIXTURE_FIRST_GUARD_FAILURE") == "1"
...
def zero_work_counters() -> dict[str, int]:
    return {
        "j1c_training_phase_locks": 0,
        "j1c_training_markers": 0,
        "j1c_materialized_manifests": 0,
        "j1c_owners": 0,
        "j1c_streams_reserved": 0,
        "j1c_streams_consumed": 0,
        "j1c_genesis_commits": 0,
...
def runtime_storage_projection() -> dict[str, Any]:
    ...
    payload = {
        ...
        "method": (
            ...
            "32-MiB J1c wrapper artifact envelope and 120-second "
...
def write_test_evidence(
    *,
    ...
    if FUTURE_EXECUTION_ROOT.exists():
        raise J1cSurfaceIntegrityError(
            "Future J1c execution root exists before test evidence"
        )
    expected_kinds = {
        "py_compile",
        "focused_j1c_surface",
    ...
    if not payload["passes"]:
        raise J1cSurfaceIntegrityError(
            "J1c training-surface test evidence gates failed"
        )
    return write_immutable_json(
...
def seal_readiness_package(
    *,
    ...
    if FUTURE_EXECUTION_ROOT.exists():
        raise J1cSurfaceIntegrityError(
            "Future J1c execution root exists before readiness seal"
        )
    paths = _readiness_paths(readiness_dir)
    for name in ("source_manifest", "stream_authority"):
        if not paths[name].is_file():
            raise J1cSurfaceIntegrityError(
                f"J1c {name} must precede readiness"
            )
    if not paths["test_evidence"].is_file():
        raise J1cSurfaceIntegrityError(
            "J1c test evidence must precede readiness"
        )
    for name in ("schema", "projection", "input_bindings", "lock", "result"):
        if paths[name].exists():
            raise FileExistsError(
                f"J1c readiness artifact already exists: {paths[name]}"
            )
    input_audit = audit_authoritative_inputs(
    ...
    if operational_audit.get("passes") is not True:
        raise J1cSurfaceOperationalHold(
            "J1c readiness operational audit failed"
        )
    if (
        ...
        raise J1cSurfaceIntegrityError(
            "J1c real operational-audit marker roundtrip is absent"
        )
    ...
    result = write_immutable_json(
        paths["result"],
        {
            ...
            "hold": "all J1c scientific training execution",
            "kill": "historical kills unchanged; J1/J1c not killed",
            "promote": False,
            "zero_work": zero_work_counters(),
            "passes": True,
        },
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="J1c training-only production dispatcher",
    )
...
def _dispatch(args: argparse.Namespace) -> dict[str, Any]:
    if args.jobs != 1:
        raise J1cSurfaceIntegrityError("J1c jobs must equal one")
    ...
    raise J1cSurfaceIntegrityError(
        f"Unsupported J1c command: {args.subcommand}"
    )
