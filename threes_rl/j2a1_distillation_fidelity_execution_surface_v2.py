"""J2 A1 distillation and closed-loop fidelity execution surface V2.
...
V2 changes only exact top-three representation canonicalization and immutable
namespacing/predecessor bindings. V1 remains preserved and non-executable.
"""
...
from threes_rl import j2a1_distillation_fidelity_execution_surface as v1


VERSION = "j2a1_distillation_fidelity_execution_surface_v2"
...
FUTURE_EXECUTION_DIR = (
    RUNS_ROOT
    / "forensics"
    / "j2a1_distillation_fidelity_execution_v2"
)
...
TEST_EVIDENCE_NAME = "J2A1_V2_EXECUTION_SURFACE_TEST_EVIDENCE.json"
INPUT_BINDINGS_NAME = "J2A1_V2_EXECUTION_SURFACE_INPUT_BINDINGS.json"
AUTHORITY_AUDIT_NAME = "J2A1_V2_EXECUTION_SURFACE_AUTHORITY_AUDIT.json"
SCHEMA_NAME = "J2A1_V2_EXECUTION_SURFACE_SCHEMA.json"
PROJECTION_NAME = "J2A1_V2_EXECUTION_SURFACE_PROJECTION.json"
READINESS_LOCK_NAME = "J2A1_V2_EXECUTION_SURFACE_READINESS_LOCK.json"
READINESS_RESULT_NAME = "J2A1_V2_EXECUTION_SURFACE_READINESS_RESULT.json"
RETENTION_NAME = "J2A1_V2_EXECUTION_SURFACE_RETENTION.json"

PHASE_LOCK_NAME = "J2A1_V2_DISTILLATION_PHASE_LOCK.json"
OPEN_MARKER_NAME = "J2A1_V2_DISTILLATION_EXECUTION_MARKER.json"
MATERIALIZED_MANIFEST_NAME = "J2A1_V2_DISTILLATION_ACTIVE_MANIFEST.json"
RESERVATION_NAME = "J2A1_V2_DISTILLATION_STREAM_RESERVATION.json"
CONSUMPTION_NAME = "J2A1_V2_DISTILLATION_STREAM_CONSUMPTION.json"
GENESIS_NAME = "J2A1_V2_DISTILLATION_GENESIS.json"
FINAL_OPERATIONAL_GUARD_NAME = (
    "J2A1_V2_DISTILLATION_FINAL_OPERATIONAL_GUARD.json"
)
TERMINAL_EVIDENCE_NAME = (
    "J2A1_V2_DISTILLATION_FIDELITY_TERMINAL_EVIDENCE.json"
)
TERMINAL_NAME = "J2A1_V2_DISTILLATION_FIDELITY_TERMINAL.json"
EXECUTION_RETENTION_NAME = "J2A1_V2_DISTILLATION_FIDELITY_RETENTION.json"

READY = "READY_J2A1_DISTILLATION_FIDELITY_EXECUTION_SURFACE_V2"
HOLD = "HOLD_J2A1_DISTILLATION_FIDELITY_EXECUTION_SURFACE_V2"
KILL = "KILL_J2A1_DISTILLATION_FIDELITY_EXECUTION_SURFACE_V2_INTEGRITY"

READY_EXECUTION = "READY_J2A1_V2_PPO_EXECUTION_SURFACE_REVIEW"
HOLD_FAMILY = "HOLD_J2A1_V2_FAMILY_DATA_SUPPORT"
HOLD_MECHANISM = "HOLD_J2A1_V2_BC_MECHANISM"
HOLD_FIDELITY = "HOLD_J2A1_V2_CLOSED_LOOP_FIDELITY"
HOLD_BASE_RATE = "HOLD_J2A1_V2_FIDELITY_INCONCLUSIVE_LOW_BASE_RATE"
HOLD_OPERATIONAL = "HOLD_J2A1_V2_DISTILLATION_OPERATIONAL"
KILL_EXECUTION = "KILL_J2A1_V2_DISTILLATION_FIDELITY_INTEGRITY"
...
DISTILLATION_AUTHORIZATION_DECISION = (
    "CONTINUE_J2A1_DISTILLATION_FIDELITY_EXECUTION_V2"
)
...
V1_AUTHORIZATION_PATH = (
    ...
    / "J2A1_DISTILLATION_FIDELITY_EXECUTION_AUTHORIZATION.json"
)
V1_EXECUTION_DIR = (
    RUNS_ROOT
    / "forensics"
...
EXPECTED_V1_READINESS_ARTIFACTS = {
    "J2A1_EXECUTION_SURFACE_AUTHORITY_AUDIT.json": (
    ...
    "J2A1_EXECUTION_SURFACE_INPUT_BINDINGS.json": (
    ...
    "J2A1_EXECUTION_SURFACE_PROJECTION.json": (
    ...
    "J2A1_EXECUTION_SURFACE_READINESS_LOCK.json": (
    ...
    "J2A1_EXECUTION_SURFACE_READINESS_RESULT.json": (
    ...
    "J2A1_EXECUTION_SURFACE_RETENTION.json": (
    ...
    "J2A1_EXECUTION_SURFACE_SCHEMA.json": (
    ...
    "J2A1_EXECUTION_SURFACE_TEST_EVIDENCE.json": (
...
def canonical_top_three(values: object) -> tuple[int, int, int]:
    if type(values) not in (list, tuple):
        raise J2A1ExecutionIntegrityError(
            "Top-three value must be an exact list or tuple"
        )
    sequence = list(values)
    if len(sequence) != 3:
        raise J2A1ExecutionIntegrityError(
            "Top-three value must contain exactly three scores"
        )
    if any(type(value) is not int for value in sequence):
        raise J2A1ExecutionIntegrityError(
            "Top-three scores must have exact integer type"
        )
    if len(set(sequence)) != 3:
        raise J2A1ExecutionIntegrityError(
            "Top-three scores must be distinct"
        )
    return (sequence[0], sequence[1], sequence[2])


def top_three_exact(live: object, expected: object) -> bool:
    try:
        live_canonical = canonical_top_three(live)
        expected_canonical = canonical_top_three(expected)
    except J2A1ExecutionIntegrityError:
        return False
    return live_canonical == expected_canonical
...
def v1_zero_work_failure_audit() -> dict[str, Any]:
    live_top_three = (263670, 261369, 258561)
    expected_top_three = [263670, 261369, 258561]
    forbidden_names = (
        v1.PHASE_LOCK_NAME,
        v1.OPEN_MARKER_NAME,
        v1.MATERIALIZED_MANIFEST_NAME,
        v1.RESERVATION_NAME,
        v1.CONSUMPTION_NAME,
        v1.GENESIS_NAME,
        v1.TERMINAL_NAME,
        v1.EXECUTION_RETENTION_NAME,
    )
    observed_files = (
        sorted(path.name for path in V1_EXECUTION_DIR.rglob("*") if path.is_file())
        if V1_EXECUTION_DIR.exists()
        else []
    )
    checks = {
        "v1_execution_root_absent": not V1_EXECUTION_DIR.exists(),
        "v1_scientific_artifacts_absent": not any(
            name in observed_files for name in forbidden_names
        ),
        "v1_raw_tuple_list_comparison_false":
            live_top_three != expected_top_three,
        "v1_values_and_order_exact_after_canonicalization":
            canonical_top_three(live_top_three)
            == canonical_top_three(expected_top_three),
        "v1_live_type_tuple": type(live_top_three) is tuple,
        "v1_expected_type_list": type(expected_top_three) is list,
        "v1_zero_work_exact": all(value == 0 for value in v1.ZERO_WORK.values()),
    }
    evidence = {
        "version": f"{VERSION}_v1_zero_work_failure_evidence_v1",
        "classification": "outcome-free orchestration representation defect",
        "v1_execution_root": str(V1_EXECUTION_DIR.resolve()),
        "observed_files": observed_files,
        "live_top_three_type": type(live_top_three).__name__,
        "expected_top_three_type": type(expected_top_three).__name__,
        "canonical_top_three": list(canonical_top_three(live_top_three)),
        "v1_zero_work": dict(v1.ZERO_WORK),
        "checks": checks,
        "passes": all(checks.values()),
    }
    return {
...
def source_and_parent_audit(
    *,
    ...
    v1_source_checks = {
        str(path.relative_to(REPO_ROOT)): sha256_path(path) == expected
        for path, expected in EXPECTED_V1_SOURCE_HASHES.items()
    }
    v1_readiness_artifacts = {}
    for name, (
        file_sha,
        field,
        payload_sha,
    ) in EXPECTED_V1_READINESS_ARTIFACTS.items():
        ...
        v1_readiness_artifacts[name] = artifact_identity(path, field)
    v1_authorization = load_bound_json(
        V1_AUTHORIZATION_PATH,
        file_sha256=EXPECTED_V1_AUTHORIZATION[0],
        payload_field=EXPECTED_V1_AUTHORIZATION[1],
        payload_sha256=EXPECTED_V1_AUTHORIZATION[2],
    )
    v1_failure = v1_zero_work_failure_audit()
    ...
    checks = {
        ...
        "v1_surface_sources_exact": all(v1_source_checks.values()),
        "v1_readiness_artifacts_exact": len(v1_readiness_artifacts)
        == len(EXPECTED_V1_READINESS_ARTIFACTS),
        "v1_authorization_exact": (
            v1_authorization.get("decision")
            == v1.DISTILLATION_AUTHORIZATION_DECISION
            and v1_authorization.get("execution_mode") == "scientific"
            and v1_authorization.get("scientific_authority") is True
            and v1_authorization.get("training_only") is True
        ),
        "v1_zero_work_failure_exact": v1_failure["passes"] is True,
        "future_authorization_absent": not FUTURE_AUTHORIZATION_DIR.exists(),
    ...
    return {
        ...
        "v1_source_checks": v1_source_checks,
        "v1_readiness_artifacts": v1_readiness_artifacts,
        "v1_authorization": artifact_identity(
            V1_AUTHORIZATION_PATH,
            EXPECTED_V1_AUTHORIZATION[1],
        ),
        "v1_zero_work_failure": v1_failure,
        "checks": checks,
        "passes": all(checks.values()),
    }
...
def execution_schema() -> dict[str, Any]:
    schema = {
        ...
        "v2_orchestration_repair": {
            "field": "dashboard.top_three",
            "live_and_expected_canonical_type": "tuple[int,int,int]",
            "exact_length": 3,
            "exact_element_type": "int",
            "distinct_scores_required": True,
            "order_preserved": True,
            "comparison": "exact canonical tuple equality",
            "all_other_semantics_inherited": True,
        },
    ...
    *,
    ...
    if include_services:
        ...
        checks = {
            ...
            is True,
            "dashboard_top_three_exact": top_three_exact(
...
def audit_zero_work(
    *,
    ...
    return {
        ...
        "future_authorization_dir": str(FUTURE_AUTHORIZATION_DIR.resolve()),
...
def execution_operational_guard(
    *,
    ...
    checks = {
        ...
        is False,
        "top_three_unchanged": top_three_exact(
            services.get("dashboard", {}).get("top_three"),
            [263670, 261369, 258561],
        ),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "J2A1 V2 distillation/fidelity readiness and bounded "
