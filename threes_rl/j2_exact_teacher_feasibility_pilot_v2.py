"""Outcome-free exact-teacher engineering feasibility pilot V2 for J2."""
...
from threes_rl import j2_exact_teacher_feasibility_pilot as v1
from threes_rl import j2_incumbent_distillation_readiness as j2


VERSION = "j2_exact_teacher_feasibility_pilot_v2"
REPO_ROOT = Path(__file__).resolve().parents[1]
CHARTER_PATH = (
    REPO_ROOT
    / "threes_rl"
    / "J2_EXACT_TEACHER_ENGINEERING_FEASIBILITY_PILOT_V2_CHARTER.md"
)
RUNNER_PATH = (
    REPO_ROOT / "threes_rl" / "j2_exact_teacher_feasibility_pilot_v2.py"
)
TEST_PATH = (
    REPO_ROOT
    / "tests"
    / "test_rl_j2_exact_teacher_feasibility_pilot_v2.py"
)
OUTPUT_DIR = (
    REPO_ROOT
    / "threes_rl"
    / "runs"
    / "forensics"
    / "j2_exact_teacher_feasibility_pilot_v2"
)

TEST_EVIDENCE_NAME = "J2_TEACHER_PILOT_V2_TEST_EVIDENCE.json"
PREFLIGHT_LOCK_NAME = "J2_TEACHER_PILOT_V2_PREFLIGHT_LOCK.json"
PREFLIGHT_RESULT_NAME = "J2_TEACHER_PILOT_V2_PREFLIGHT_RESULT.json"
MARKER_NAME = "J2_TEACHER_PILOT_V2_EXECUTION_MARKER.json"
INVENTORY_NAME = "J2_TEACHER_PILOT_V2_STATE_INVENTORY.json"
CENTRAL_NAME = "J2_TEACHER_PILOT_V2_CENTRAL_COST.json"
SENSITIVITY_NAME = "J2_TEACHER_PILOT_V2_SENSITIVITY_COST.json"
SYNC_NAME = "J2_TEACHER_PILOT_V2_SYNCHRONOUS_ORCHESTRATION.json"
POWER_NAME = "J2_TEACHER_PILOT_V2_POWER_SIZING.json"
TERMINAL_NAME = "J2_TEACHER_PILOT_V2_TERMINAL_RESULT.json"
RETENTION_NAME = "J2_TEACHER_PILOT_V2_RETENTION.json"
ROUND_DIR_NAME = "round_manifests"

READY_PREFLIGHT = "READY_J2_TEACHER_FEASIBILITY_PILOT_V2_EXECUTION"
HOLD_PREFLIGHT = "HOLD_J2_TEACHER_FEASIBILITY_PILOT_V2_PREFLIGHT"
READY_TERMINAL = "READY_J2_FEASIBILITY_AMENDMENT_PREFLIGHT_V2"
HOLD_TERMINAL = "HOLD_J2_TEACHER_ENGINEERING_FEASIBILITY_V2"
KILL_TERMINAL = "KILL_J2_TEACHER_PILOT_V2_INTEGRITY"
...
WORKERS = 8
WARMUP_CALLS = 8
DECK_BASE = 253_000_000_000
SLOT_BASE = 254_000_000_000
EXPLORATION_BASE = 255_000_000_000
PREFIX_MIN = 16
PREFIX_SPAN = 160
...
V1_TEST_EVIDENCE_NAME = "J2_TEACHER_PILOT_TEST_EVIDENCE.json"
...
EXPECTED_V1_EVIDENCE_FILE_SHA256 = (
    "ec9235912266c0e18e7596d08cdca4468f2605888c6a6de1c165224c17e8aee3"
)
EXPECTED_V1_EVIDENCE_PAYLOAD_SHA256 = (
...
def v1_history_audit() -> dict[str, Any]:
    sources = {}
    for name, (path, expected) in EXPECTED_V1_IDENTITIES.items():
    ...
    evidence_path = V1_OUTPUT_DIR / V1_TEST_EVIDENCE_NAME
    evidence = load_hashed_json(
        evidence_path,
        field="test_evidence_payload_sha256",
        file_sha256=EXPECTED_V1_EVIDENCE_FILE_SHA256,
        payload_sha256=EXPECTED_V1_EVIDENCE_PAYLOAD_SHA256,
    )
    observed_files = (
        sorted(path.name for path in V1_OUTPUT_DIR.iterdir() if path.is_file())
        if V1_OUTPUT_DIR.is_dir()
        else []
    )
    checks = {
        "v1_sources_exact": all(row["passes"] for row in sources.values()),
        "v1_namespace_exact": observed_files == [V1_TEST_EVIDENCE_NAME],
        "v1_evidence_zero_failed": int(evidence["total_failed"]) == 0,
        "v1_evidence_zero_teacher_queries": (
            int(evidence["zero_work"]["teacher_queries"]) == 0
        ),
        "v1_evidence_zero_games": int(evidence["zero_work"]["games"]) == 0,
        "v1_inventory_absent": not (
            V1_OUTPUT_DIR / "J2_TEACHER_PILOT_STATE_INVENTORY.json"
        ).exists(),
        "v1_lock_absent": not (
            V1_OUTPUT_DIR / "J2_TEACHER_PILOT_PREFLIGHT_LOCK.json"
        ).exists(),
        "v1_result_absent": not (
            V1_OUTPUT_DIR / "J2_TEACHER_PILOT_PREFLIGHT_RESULT.json"
        ).exists(),
        "v1_marker_absent": not (
            V1_OUTPUT_DIR / "J2_TEACHER_PILOT_EXECUTION_MARKER.json"
        ).exists(),
        "v1_terminal_absent": not (
            V1_OUTPUT_DIR / "J2_TEACHER_PILOT_TERMINAL_RESULT.json"
        ).exists(),
        "v1_retention_absent": not (
            V1_OUTPUT_DIR / "J2_TEACHER_PILOT_RETENTION.json"
        ).exists(),
    }
    return {
        "version": f"{VERSION}_v1_history_audit_v1",
        "adjudication": "HOLD_J2_TEACHER_PILOT_V1_PREFLIGHT",
        "sources": sources,
        "evidence": {
...
def source_identity_audit() -> dict[str, Any]:
    ...
    checks = {
        ...
        == j2.HOLD,
        "v1_history_exact_and_hold": v1_history["passes"],
    }
    return {
...
def stream_authority_audit() -> dict[str, Any]:
    ...
    v1_rows = v1.engineering_stream_rows()
    roots = [str(row["root_id"]) for row in rows]
    ancestries = [str(row["ancestry_id"]) for row in rows]
    v1_roots = {str(row["root_id"]) for row in v1_rows}
    v1_ancestries = {str(row["ancestry_id"]) for row in v1_rows}
    stream_ids = [
        int(value)
        for row in rows
    ...
    checks = {
        ...
        "engineering_prefixes_exact": prefixes == {253, 254, 255},
        ...
        "no_v1_250b_252b_collision": not (
            prefixes & {250, 251, 252}
        ),
        "no_v1_root_collision": not (set(roots) & v1_roots),
        "no_v1_ancestry_collision": not (
            set(ancestries) & v1_ancestries
        ),
...
def target_prefix_band(target_prefix_steps: int) -> str:
    target = int(target_prefix_steps)
    for name, lower, upper in PREFIX_BANDS:
        if lower <= target <= upper:
            return name
    raise PilotIntegrityError("Target prefix is outside frozen V2 bands")
...
def _state_from_engineering_row(
    ...
    initial_legal = sim.legal_actions(state)
    if sim.starter_tile is not None or state.game_over or not initial_legal:
        raise PilotIntegrityError(
            "Engineering ancestry has no live normal-start state"
        )
    ...
    for _ in range(target):
        ...
        if state.game_over or not legal:
            raise PilotIntegrityError(
                "Retained engineering state ceased to be live"
            )
    ...
    if (
        ...
        or not legal
        or reached > target
        or prefix_clamped != (reached < target)
    ):
        raise PilotIntegrityError("Engineering state is not a total live prefix")
    snapshot = j1.simulator_snapshot(sim, state)
    state_hash = canonical_hash(snapshot)
    manifest = {
        **dict(row),
        "realized_prefix_steps": reached,
        "prefix_clamped": prefix_clamped,
...
def clamp_summary(
    ...
    band_counts = {name: 0 for name, _lower, _upper in PREFIX_BANDS}
    band_clamped = {name: 0 for name, _lower, _upper in PREFIX_BANDS}
    family_counts = {family: 0 for family in j2.FEATURE_FAMILIES}
    family_clamped = {family: 0 for family in j2.FEATURE_FAMILIES}
    joint_counts = {
        name: {family: 0 for family in j2.FEATURE_FAMILIES}
        for name, _lower, _upper in PREFIX_BANDS
    }
    joint_clamped = {
        name: {family: 0 for family in j2.FEATURE_FAMILIES}
        for name, _lower, _upper in PREFIX_BANDS
    ...
    for row in manifest_rows:
        band = target_prefix_band(int(row["target_prefix_steps"]))
        family = str(row["feature_family"])
        if family not in family_counts:
            raise PilotIntegrityError("Unknown feature family in inventory")
        clamped = bool(row["prefix_clamped"])
        band_counts[band] += 1
        family_counts[family] += 1
        joint_counts[band][family] += 1
        if clamped:
            band_clamped[band] += 1
            family_clamped[family] += 1
            joint_clamped[band][family] += 1

    def cell(count: int, clamped: int) -> dict[str, Any]:
        return {
            "root_count": int(count),
            "clamped_root_count": int(clamped),
            "clamping_rate": (
                float(clamped / count) if count else 0.0
            ),
        }

    total = len(manifest_rows)
    total_clamped = sum(bool(row["prefix_clamped"]) for row in manifest_rows)
    return {
        "total": cell(total, total_clamped),
        "by_target_prefix_band": {
            name: cell(band_counts[name], band_clamped[name])
            for name, _lower, _upper in PREFIX_BANDS
        },
        "by_feature_family": {
            family: cell(family_counts[family], family_clamped[family])
            for family in j2.FEATURE_FAMILIES
        },
        "by_target_prefix_band_and_feature_family": {
            name: {
                family: cell(
                    joint_counts[name][family],
                    joint_clamped[name][family],
                )
                for family in j2.FEATURE_FAMILIES
            }
            for name, _lower, _upper in PREFIX_BANDS
...
def build_and_seal_inventory(
    *,
    ...
    clamping = clamp_summary(manifest_rows)
    checks = {
        ...
        "all_prefixes_total": all(
            (
                bool(row["prefix_clamped"])
                and 0
                <= int(row["realized_prefix_steps"])
                < int(row["target_prefix_steps"])
            )
            or (
                row["prefix_clamped"] is False
                and int(row["realized_prefix_steps"])
                == int(row["target_prefix_steps"])
            )
            for row in manifest_rows
        ),
        "all_planned_roots_retained_once": (
            observed_identities == planned_identities
        ),
        "roots_replaced_zero": observed_identities == planned_identities,
        "roots_dropped_zero": len(observed_identities) == len(planned_identities),
        "survival_conditioning_zero": (
            [int(row["state_index"]) for row in manifest_rows]
            == list(range(INVENTORY_COUNT))
        ),
        "clamp_accounting_total": (
            int(clamping["total"]["root_count"]) == INVENTORY_COUNT
        ),
    ...
    lock = {
        ...
        "inventory_contract": {
            ...
            "early_termination_rule": (
                "retain_final_live_preterminal_state_without_replacement"
            ),
            "every_root_retained_exactly_once": True,
            "survival_conditioning": False,
            "warmup_calls_per_process": WARMUP_CALLS,
        },
        ...
        "execution_command": (
            ...
            "threes_rl.j2_exact_teacher_feasibility_pilot_v2 execute "
            "--out-dir "
