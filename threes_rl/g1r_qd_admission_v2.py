"""Outcome-free admission for the G1-R QD v2 terminal-schema family."""
...
VERSION = "g1r_qd_admission_v2_terminal_schema"
PROPOSAL_PATH = Path("threes_rl/G1R_QD_V2_TERMINAL_SCHEMA_PROPOSAL.md")
PROPOSAL_SHA256 = (
    "9a95f0b9a60c1825b9f083321c0bfd4048290e2e60893bdff5e9b0bac98789cf"
)
CHARTER_PATH = Path("threes_rl/G1R_QD_V2_ADMISSION_EXECUTION_CHARTER.md")
IMPLEMENTATION_PATH = Path("threes_rl/g1r_qd_admission_v2.py")
TEST_PATH = Path("tests/test_rl_g1r_qd_admission_v2.py")
...
DESCRIPTOR_VERSION = "g1r_qd_descriptor_v2_terminal_legal0"
ARCHIVE_VERSION = "g1r_qd_archive_v2_terminal_legal0"
SOURCE_MANIFEST_VERSION = "g1r_qd_archive_sources_v2_terminal_legal0"
POLICY_VERSION = "g1r_qd_policy_v2_terminal_legal0"
FAMILY_NAME = "g1r_qd_static_archive_oneply_v2_terminal_schema"
...
DESCRIPTOR_SCHEMA = {
    ...
    "legal_action_count_domain": [0, 1, 2, 3, 4],
}
...
def board_descriptor(state: SimState, sim: ThreesSim) -> tuple[int, ...]:
    ...
    if not 0 <= legal_count <= 4:
        raise ValueError(f"Descriptor legal count outside 0..4: {legal_count}")
    descriptor = (
...
def enumerate_exact_insertion_descriptors(
    state: SimState,
    sim: ThreesSim,
) -> list[dict[str, Any]]:
    """Enumerate descriptors only; never rank, value, or select an action."""

    rows: list[dict[str, Any]] = []
    for action in sim.legal_actions(state):
        shifted, eligible_positions = simulate_base_move(state.board, action)
        if not eligible_positions:
            raise ValueError(f"Legal action has no insertion positions: {action}")
        ...
        for position in eligible_positions:
            for inserted_value, value_probability in insert_options:
                ...
                legal_count = len(sim.legal_actions(outcome_state))
                if descriptor[7] != legal_count:
                    raise AssertionError("Descriptor legal count disagrees with simulator")
                rows.append(
                    {
                        ...
                        "terminal": legal_count == 0,
                    }
                )
                probability_total += probability
        if not np.isclose(probability_total, 1.0, atol=1e-12):
            raise ValueError(
                f"Insertion probabilities sum to {probability_total} for {action}"
            )
    return rows
...
class StaticArchive:
    def __init__(self, counts: dict[tuple[int, ...], int]) -> None:
        ...
        for key, count in counts.items():
            ...
            if not 0 <= cell[7] <= 4:
                raise ValueError("Archive legal count outside 0..4")
            if int(count) <= 0:
...
class StaticArchiveQDPolicy:
    ...
    def load(cls, path: Path) -> "StaticArchiveQDPolicy":
        ...
        if meta.get("version") != POLICY_VERSION:
...
def _load_and_validate_lock(out_dir: Path) -> dict[str, Any]:
    ...
    if lock.get("version") != VERSION:
        raise ValueError("QD execution lock version mismatch")
...
def _execute_opened_admission(
    *,
    ...
    for index, (_record, state, sim) in enumerate(states):
        ...
        signatures[FAMILY_NAME].append(
            int(qd_decision["action"])
        )
        tie_counts[FAMILY_NAME] += int(
    ...
    pairwise = []
    candidate_signature = signatures[FAMILY_NAME]
    for family in REFERENCE_FAMILIES:
