"""G3 E0 v3 open/resume orchestration around the frozen scientific core.
...
RUNNER_PATH = Path("threes_rl/g3_e0_label_fit_v3.py")
OUTPUT_DIR = Path("threes_rl/runs/forensics/g3_e0_label_fit_v3")
OPEN_MARKER_NAME = core.OPEN_MARKER_NAME
TERMINAL_RESULT_NAME = core.TERMINAL_RESULT_NAME
PREFLIGHT_DECISION = "READY_G3_E0_V3_EXECUTION"
OPEN_COMMAND = (
    "zsh -ic 'no-secrets nice -n 10 env PYTHONPATH=. "
    ".venv/bin/python -m threes_rl.g3_e0_label_fit_v3 open "
    "--out-dir threes_rl/runs/forensics/g3_e0_label_fit_v3 "
    "--preflight-lock "
    "threes_rl/runs/forensics/g3_e0_label_fit_v3/preflight_lock.json "
    "--jobs 1'"
)
EXECUTE_COMMAND = (
    "zsh -ic 'no-secrets nice -n 10 env PYTHONPATH=. "
    ".venv/bin/python -m threes_rl.g3_e0_label_fit_v3 execute "
    "--out-dir threes_rl/runs/forensics/g3_e0_label_fit_v3 "
    "--preflight-lock "
    "threes_rl/runs/forensics/g3_e0_label_fit_v3/preflight_lock.json "
    "--jobs 1'"
...
def _load_lock(
    ...
    *,
    ...
    if jobs != 1:
        raise ValueError("Frozen G3 E0 v3 execution requires jobs=1")
    ...
    if lock.get("decision") != PREFLIGHT_DECISION:
        raise PermissionError("V3 preflight did not authorize execution")
...
def _stream_collision_audit(
    ...
    *,
    ...
    audit = preflight_core.historical_collision_audit(
        tasks,
        excluded_exact_directories=excluded,
    )
    expected = lock["stream_collision_lock"]
    checks = {
        "collision_passes": audit["passes"],
        "matched_source_count_exact":
            int(audit["matched_source_count"])
            == int(expected["matched_source_count"]),
        "matched_sources_sha256_exact":
            str(audit["matched_sources_sha256"])
            == str(expected["matched_sources_sha256"]),
        "excluded_directories_exact":
...
def _immutable_contract(
    ...
    *,
    ...
    return {
        ...
        "v3_orchestration_amendment_sha256": AMENDMENT_SHA256,
        ...
        "stream_collision_lock": dict(lock["stream_collision_lock"]),
    }
...
def _assert_zero_work(out_dir: Path) -> None:
    ...
    if present or unexpected:
        raise FileExistsError(
            f"V3 output is not zero-work: present={present}, "
...
def open_execution(
    *,
    ...
    if not (
        ...
        raise RuntimeError("V3 open-only revalidation failed")
    ...
    payload = core.payload_with_hash(
        {
            "version": VERSION,
            "decision": "OPENED_G3_E0_V3_EXECUTION",
...
def validate_execution_marker(
    *,
    ...
    if not marker_path.is_file():
        raise FileNotFoundError("Missing G3 E0 v3 execution marker")
    if (out_dir / TERMINAL_RESULT_NAME).exists():
        raise FileExistsError("G3 E0 v3 terminal result is immutable")
    ...
    if marker.get("decision") != "OPENED_G3_E0_V3_EXECUTION":
