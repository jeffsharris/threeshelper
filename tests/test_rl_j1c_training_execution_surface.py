...
from threes_rl import j1c_training_execution_surface as surface
...
def _fixture_rows(count: int = 2) -> list[dict]:
    ...
    for index in range(count):
        root_id = hashlib.sha256(
            f"j1c-fixture-root-{index}".encode("ascii")
        ).hexdigest()
        row = {
            "phase": "training",
            "partition": "train",
...
def test_module_import_is_torch_and_parent_light() -> None:
    code = (
        "import json,sys;"
        "import threes_rl.j1c_training_execution_surface as module;"
...
def test_post_write_equal_object_changed_bytes_fail_closed(
    ...
    with pytest.raises(
        surface.J1cSurfaceIntegrityError,
        match="changed bytes",
    ):
...
def test_authoritative_j1c_inputs_reproduce_exactly() -> None:
...
@pytest.mark.parametrize(
    "failure_env",
    [
        {"J1C_FIXTURE_RUNTIME_FAILURE": "1"},
        {"J1C_FIXTURE_FIRST_GUARD_FAILURE": "1"},
    ],
)
...
def test_clean_subprocess_real_operational_audit_marker_roundtrip(
    ...
    code = (
        "import json;"
        "from pathlib import Path;"
        "from threes_rl import j1c_training_execution_surface as s;"
...
def test_fixture_full_chain_routes_bounded_training_only(
    ...
    assert payload["terminal_decision"] == (
        "READY_J1C_MINIATURE_TRAINING_FIXTURE"
    )
...
def test_pre_engine_crash_reclaims_same_contract_without_duplicate_streams(
    ...
    interrupted = _cli(
        ...
        readiness,
        execution,
        env_add={"J1C_FIXTURE_PRE_ENGINE_INTERRUPT": boundary},
    )
...
def test_collection_update_checkpoint_resume_is_bit_exact(
    ...
    for boundary in ("collection", "update", "checkpoint"):
        interrupted = _cli(
            ...
            resumed_execution,
            env_add={
                "J1C_FIXTURE_INTERRUPT_AFTER_BOUNDARY": boundary
            },
        )
...
def test_changed_source_manifest_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(surface.J1cSurfaceIntegrityError):
...
def test_prospective_zero_work_counters_are_j1c_labelled() -> None:
    counters = surface.zero_work_counters()
    assert counters
    assert all(value == 0 for value in counters.values())
    assert not any(key.startswith("j1b_") for key in counters)
    assert {
        "j1c_training_phase_locks",
        "j1c_training_markers",
        "j1c_materialized_manifests",
        "j1c_owners",
        "j1c_streams_reserved",
        "j1c_streams_consumed",
        "j1c_genesis_commits",
    }.issubset(counters)
...
def test_scientific_fixture_controls_are_rejected_before_work(
    ...
    monkeypatch.setenv("J1C_FIXTURE_RUNTIME_FAILURE", "1")
    assert os.environ["J1C_FIXTURE_RUNTIME_FAILURE"] == "1"
...
def test_runner_can_be_loaded_without_invoking_cli() -> None:
    namespace = runpy.run_path(
        surface.RUNNER_PATH,
        run_name="j1c_surface_import_fixture",
    )
