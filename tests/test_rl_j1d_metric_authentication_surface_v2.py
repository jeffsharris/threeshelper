...
from threes_rl import j1d_metric_authentication_surface_v2 as surface
...
def test_v1_pre_correction_evidence_is_immutable() -> None:
    for relative, expected in sorted(
        surface.EXPECTED_V1_SOURCE_IDENTITIES.items()
    ):
        assert surface.sha256_path(surface.REPO_ROOT / relative) == expected
    assert sorted(
        path.name
        for path in surface.V1_READINESS_DIR.iterdir()
        if path.is_file()
    ) == sorted(surface.EXPECTED_V1_READINESS_EVIDENCE)
    for name, (
        file_sha256,
        payload_field,
        payload_sha256,
    ) in sorted(surface.EXPECTED_V1_READINESS_EVIDENCE.items()):
        path = surface.V1_READINESS_DIR / name
        payload = surface.load_json(path)
        assert surface.sha256_path(path) == file_sha256
        assert surface.verify_payload_hash(payload, payload_field)
        assert payload[payload_field] == payload_sha256
    assert not (
        surface.V1_READINESS_DIR
...
def test_module_import_is_torch_and_parent_light() -> None:
    code = (
        "import json,sys;"
        "import threes_rl.j1d_metric_authentication_surface_v2 as module;"
...
def test_production_cast_reproduces_reduction_order_failure() -> None:
    mechanism = surface.reproduce_legacy_and_canonical_reductions(
        lengths=[1, 7, 31],
        per_root_values=[0.0, 1.0, 0.0],
    )
    assert mechanism["parent_weight_dtype"] == "float64"
    assert mechanism["production_batch_weight_dtype"] == "float32"
    assert mechanism["metric_reduction_weight_dtype"] == "float64"
    assert mechanism["root_weight_totals"] == [
        1.0,
        1.0000000447034836,
        0.9999999701976776,
    ]
    assert mechanism["legacy_direct_global"] == 0.33333334657880986
    assert mechanism["canonical_per_root_mean"] == 1.0 / 3.0
    assert mechanism["absolute_delta"] == 1.324547654890651e-08
    assert mechanism["exceeds_frozen_tolerance"]
    assert mechanism["computed_without_injected_delta"]

    row = _metric_round_fixture()
    legacy = json.loads(json.dumps(row))
    per_root_values = [0.0, 1.0, 0.0]
    for root_metric, length, value in zip(
        legacy["root_metrics"],
        [1, 7, 31],
        per_root_values,
    ):
        root_metric["transition_rows"] = length
        root_metric["legal_entropy_nats"] = value
        root_metric["auxiliary_brier"] = [value, value, value]
        root_metric["auxiliary_prevalence"] = [value, value, value]
    legacy["root_metrics_sha256"] = parent.j1.stable_hash(
        legacy["root_metrics"]
    )
    legacy["legal_entropy_nats"] = mechanism["legacy_direct_global"]
    legacy["auxiliary_brier"] = [
        mechanism["legacy_direct_global"]
    ] * 3
    legacy_prevalence = mechanism["legacy_direct_global"]
    legacy["auxiliary_prevalence_brier"] = [
        legacy_prevalence * (1.0 - legacy_prevalence)
    ] * 3
    canonical = surface.canonical_root_equal_round_aggregates(
        legacy["root_metrics"]
    )
...
@pytest.mark.parametrize(
    "field,index",
    [
        ("legal_entropy_nats", None),
        ("auxiliary_brier", 1),
    ],
)
def test_rehashed_one_ulp_published_tamper_fails_exactly(
    field: str,
    index: int | None,
) -> None:
    ...
    if index is None:
        tampered[field] = math.nextafter(
            float(tampered[field]),
            math.inf,
        )
    else:
        tampered[field][index] = math.nextafter(
            float(tampered[field][index]),
            math.inf,
        )
    authentication = dict(tampered["metric_authentication"])
    authentication.pop("metric_authentication_payload_sha256")
    authentication["published_projection_sha256"] = (
        surface.canonical_json_hash(surface._metric_projection(tampered))
    )
    tampered["metric_authentication"] = surface.payload_with_hash(
        authentication,
        "metric_authentication_payload_sha256",
    )
    audit = surface.validate_canonical_round_metric_row(tampered)
    assert audit["checks"]["authentication_hash_valid"]
    assert audit["checks"]["canonical_hash_exact"]
    assert audit["checks"]["published_projection_hash_exact"]
    assert not audit["checks"]["published_hash_equals_canonical_hash"]
    assert not audit["checks"]["published_projection_exact"]
    assert not audit["passes"]

    fully_rehashed = json.loads(json.dumps(tampered))
    authentication = dict(fully_rehashed["metric_authentication"])
    authentication.pop("metric_authentication_payload_sha256")
    authentication["canonical_aggregates_sha256"] = (
        authentication["published_projection_sha256"]
    )
    fully_rehashed["metric_authentication"] = surface.payload_with_hash(
        authentication,
        "metric_authentication_payload_sha256",
    )
    audit = surface.validate_canonical_round_metric_row(fully_rehashed)
    assert audit["checks"]["authentication_hash_valid"]
    assert audit["checks"]["published_projection_hash_exact"]
    assert audit["checks"]["published_hash_equals_canonical_hash"]
    assert not audit["checks"]["canonical_hash_exact"]
    assert not audit["checks"]["published_projection_exact"]
    assert not audit["passes"]
...
def test_canonical_metric_single_bit_and_field_tamper_fail_closed() -> None:
    ...
    assert (
        row["metric_authentication"]["published_projection_sha256"]
        == row["metric_authentication"]["canonical_aggregates_sha256"]
        == surface.canonical_json_hash(surface._metric_projection(row))
    )
...
def test_historical_structural_attribution_remains_separate() -> None:
    evidence = surface.root_cause_evidence()
    assert evidence["maximum_absolute_discrepancies"] == {
    ...
    assert evidence["checkpoint_or_episode_body_reads"] == 0
    assert evidence["scientific_interpretation"] is False
    assert evidence["preserved_v1_pre_correction_evidence"][
        "readiness_terminal_sealed"
    ] is False
...
def test_clean_subprocess_real_operational_audit_marker_roundtrip(
    ...
    code = (
        "import json;"
        "from pathlib import Path;"
        "from threes_rl import j1d_metric_authentication_surface_v2 as s;"
