...
from threes_rl import (
    j2a1_distillation_fidelity_execution_surface as v1_surface,
)
from threes_rl import (
    j2a1_distillation_fidelity_execution_surface_v2 as surface,
)
...
def test_v2_top_three_equal_tuple_and_list_pass() -> None:
    expected = [263670, 261369, 258561]
    assert surface.canonical_top_three(tuple(expected)) == tuple(expected)
    assert surface.canonical_top_three(expected) == tuple(expected)
    assert surface.top_three_exact(tuple(expected), expected)
    assert surface.top_three_exact(expected, tuple(expected))


@pytest.mark.parametrize(
    "observed",
    [
        (263671, 261369, 258561),
        (263670, 261370, 258561),
        (263670, 261369, 258562),
    ],
)
def test_v2_top_three_score_mutation_fails(
    observed: tuple[int, int, int],
) -> None:
    assert not surface.top_three_exact(
        observed,
...
@pytest.mark.parametrize(
    "observed",
    [
        (261369, 263670, 258561),
        (263670, 258561, 261369),
        (258561, 261369, 263670),
    ],
)
def test_v2_top_three_order_mutation_fails(
    observed: tuple[int, int, int],
) -> None:
    assert not surface.top_three_exact(
        observed,
...
@pytest.mark.parametrize(
    "observed",
    [
        [263670, 263670, 258561],
        [263670, 261369],
        [263670, 261369, 258561, 1],
        [263670, 261369, True],
        [263670, 261369, 258561.0],
        {263670: 1, 261369: 2, 258561: 3},
        {263670, 261369, 258561},
        "263670,261369,258561",
        b"263670,261369,258561",
        _ListSubclass([263670, 261369, 258561]),
    ],
)
def test_v2_top_three_duplicate_length_and_type_ambiguity_fails(
    observed: object,
) -> None:
    with pytest.raises(surface.J2A1ExecutionIntegrityError):
        surface.canonical_top_three(observed)
    assert not surface.top_three_exact(
        observed,
        [263670, 261369, 258561],
    )
...
def _production_service_fixture() -> dict[str, object]:
    return {
        "passes": True,
        "dashboard": {
...
def test_v1_production_tuple_list_rejects_and_v2_passes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from threes_rl import j1_joint_policy_value as j1

    monkeypatch.setattr(j1, "free_disk_gib", lambda: 140.0)
    monkeypatch.setattr(
        j1,
        "heavy_process_audit",
        lambda: {"passes": True},
    )
    monkeypatch.setattr(j1, "service_audit", _production_service_fixture)
    monkeypatch.setattr(os, "getpriority", lambda which, who: 10)
    monkeypatch.setattr(
        v1_surface,
    ...
    with pytest.raises(v1_surface.J2A1ExecutionOperationalHold):
        v1_surface.execution_operational_guard(
            phase_dir=tmp_path / "v1",
            accountant=v1_surface.OutputAccountant(tmp_path / "v1"),
            active_seconds=0.0,
            include_services=True,
            require_target_disk=True,
        )
    observed = surface.execution_operational_guard(
        phase_dir=tmp_path / "v2",
        accountant=surface.OutputAccountant(tmp_path / "v2"),
        active_seconds=0.0,
        include_services=True,
        require_target_disk=True,
    )
    assert observed["checks"]["top_three_unchanged"]
    assert observed["passes"]


def test_v2_binds_v1_hold_and_both_execution_roots_are_absent() -> None:
    audit = surface.source_and_parent_audit(
        require_future_execution_absent=True,
    )
    assert audit["checks"]["v1_surface_sources_exact"]
    assert audit["checks"]["v1_readiness_artifacts_exact"]
    assert audit["checks"]["v1_authorization_exact"]
    assert audit["checks"]["v1_zero_work_failure_exact"]
    assert audit["v1_zero_work_failure"]["passes"]
    assert not surface.V1_EXECUTION_DIR.exists()
...
def test_v2_inherited_function_and_class_sources_are_exact() -> None:
    permitted_function_changes = {
        "canonical_top_three",
        "top_three_exact",
        "v1_zero_work_failure_audit",
    ...
    v1_functions = {
        name: value
        for name, value in vars(v1_surface).items()
        if inspect.isfunction(value)
        and value.__module__ == v1_surface.__name__
    }
    v2_functions = {
        name: value
        for name, value in vars(surface).items()
        if inspect.isfunction(value)
        and value.__module__ == surface.__name__
    }
    for name in sorted(set(v1_functions) & set(v2_functions)):
        if name in permitted_function_changes:
            continue
        assert inspect.getsource(v2_functions[name]) == inspect.getsource(
            v1_functions[name]
        ), name
    ...
    for name in sorted(v1_classes):
        assert inspect.getsource(v2_classes[name]) == inspect.getsource(
            v1_classes[name]
        ), name


def test_v2_scientific_constants_authority_and_projection_are_exact() -> None:
    scientific_constants = (
        ...
        "SHARDS",
        "VALIDATION_STRATA",
        "VALIDATION_PAIRS_PER_STRATUM",
        "MAX_MOVES",
        "BOOTSTRAP_REPLICATES",
        "SCORE_BOOTSTRAP_SEED",
        "PROGRESSION_BOOTSTRAP_SEED",
        "BOOTSTRAP_QUANTILES",
        "QUANTILE_METHOD",
        "FEATURE_FAMILIES",
        "MIN_STATES_PER_FAMILY",
        ...
        "INHERITED_ADMIN_HOURS",
        "ACTOR_BATCH16_P99_SECONDS",
        "SIM_TRANSITION_P99_SECONDS",
        "ZERO_WORK",
    )
    for name in scientific_constants:
        assert getattr(surface, name) == getattr(v1_surface, name), name
    assert surface.expected_active_rows() == v1_surface.expected_active_rows()

    v1_authority = dict(v1_surface.authority_audit())
    v2_authority = dict(surface.authority_audit())
    v1_authority.pop("version")
    v2_authority.pop("version")
    assert v2_authority == v1_authority

    v1_projection = dict(v1_surface.runtime_storage_projection())
    v2_projection = dict(surface.runtime_storage_projection())
    v1_projection.pop("version")
    v2_projection.pop("version")
    assert v2_projection == v1_projection
