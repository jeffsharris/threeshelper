...
from threes_rl import j2_exact_teacher_feasibility_pilot as pilot
...
def test_engineering_stream_authority_is_disjoint_and_exact() -> None:
    ...
    assert report["intervals"] == {
        "deck": [250_000_000_000, 250_000_004_999],
        "slot": [251_000_000_000, 251_000_004_999],
        "exploration": [252_000_000_000, 252_000_004_999],
    }
    assert report["checks"]["no_spent_213b_226b_collision"]
    assert report["checks"]["no_j2_227b_249b_collision"]


def test_reachable_state_manifest_is_feature_only() -> None:
    row = min(
    ...
    assert manifest["reached_prefix_steps"] == 16
    assert manifest["stopped_before_target"] is False
...
def test_natural_early_termination_fails_without_replacement(
    ...
    with pytest.raises(
        ...
        pilot._state_from_engineering_row(
            {
                "state_index": 0,
                "root_id": "r",
                "ancestry_id": "a",
