...
from threes_rl import g3_e0_label_fit_v4 as v4
from threes_rl import g3_e0_preflight_v4 as preflight
...
def _collision_tasks() -> list[dict[str, Any]]:
    return [
        {
            "task_key": f"task-{index:04d}",
            "logical_seed": 57_000_000_000 + index,
            "deck_stream_id": 58_000_000_000 + index,
            "slot_stream_id": 59_000_000_000 + index,
            "policy_stream_id": 60_000_000_000 + index,
        }
        for index in range(5_072)
    ]


def _write_history(path: Path, **values: int) -> None:
...
def _collision_fixture(
    ...
    live = (
        scan_root / "dashboard" / "dashboard.json",
        scan_root / "dashboard" / "score_trends.json",
    )
    internal = (scan_root / "internal-reservation",)
    immutable = scan_root / "historical" / "manifest.json"
    _write_history(live[0], logical_seed=11)
    _write_history(live[1])
    _write_history(immutable, slot_stream_id=13)
    tasks = _collision_tasks()
    _write_history(
        internal[0] / "requested-copy.json",
        logical_seed=tasks[0]["logical_seed"],
        deck_stream_id=tasks[0]["deck_stream_id"],
    )
    manifest = v4.build_collision_source_manifest(
        tasks,
        excluded_directories=internal,
        live_source_paths=live,
        scan_root=scan_root,
    )
    assert manifest["passes"], manifest
    return scan_root, live, internal, tasks, manifest


def _lock_fixture(tmp_path: Path) -> tuple[Path, Path, dict[str, Any]]:
    out_dir = tmp_path / "g3_e0_label_fit_v4"
    out_dir.mkdir()
    ...
    collision = _write_payload(
        out_dir / v4.COLLISION_MANIFEST_NAME,
        {
            "version": "fixture-collision",
            "requested_stream_contract": {"task_rows": 0},
            "immutable_inventory_sha256": "immutable",
        },
    )
    lock = core.payload_with_hash(
        {
            "version": preflight.VERSION,
            "decision": v4.PREFLIGHT_DECISION,
            "out_dir_resolved": str(out_dir.resolve()),
            "open_command": v4.OPEN_COMMAND,
            "execution_command": v4.EXECUTE_COMMAND,
            "jobs": 1,
            "nice": 10,
            ...
            "orchestration_runner_sha256": sha256_path(v4.RUNNER_PATH),
            ...
            "collision_manifest_name": v4.COLLISION_MANIFEST_NAME,
            "collision_manifest_file_sha256": sha256_path(
                out_dir / v4.COLLISION_MANIFEST_NAME
            ),
            "collision_manifest_payload_sha256":
                collision["canonical_payload_sha256"],
            "requested_stream_contract_sha256": "requested",
            "immutable_inventory_sha256": "immutable",
            "live_source_paths": [
                str(path) for path in v4.LIVE_SOURCE_PATHS
            ],
            "live_source_count": 2,
...
def _patch_open_audits(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(v4, "_verify_bound_files", lambda _lock: _pass_audit())
    monkeypatch.setattr(
        v4,
        "_manifest_audit",
        lambda _lock, *, out_dir: _pass_audit(),
    )
    monkeypatch.setattr(v4, "_incumbent_audit", lambda _lock: _pass_audit())
    monkeypatch.setattr(
        v4,
...
def test_open_only_seals_exact_contract_and_creates_no_work(
    ...
    marker = v4.open_execution(
        out_dir=out_dir,
        preflight_lock=lock_path,
        jobs=1,
    )
    assert core.verify_payload_hash(marker)
    assert marker["contract"]["open_command"] == v4.OPEN_COMMAND
    assert marker["contract"]["execution_command"] == v4.EXECUTE_COMMAND
    ...
    assert sorted(path.name for path in out_dir.iterdir()) == sorted(
        v4.ALLOWED_BASE_FILES | {v4.OPEN_MARKER_NAME}
    )
...
def test_marker_missing_and_jobs_mismatch_fail_before_execute(
    ...
    monkeypatch.setattr(v4, "_execute_pipeline", forbidden)
...
def test_marker_contract_mismatch_rejected_before_execute(
    ...
    marker_path = out_dir / v4.OPEN_MARKER_NAME
    marker_path.unlink()
...
def test_same_marker_validates_twice_without_mutation(
    ...
    first_lock, first = v4.validate_execution_marker(
        out_dir=out_dir,
        preflight_lock=lock_path,
        jobs=1,
        revalidate_operations=False,
    )
    second_lock, second = v4.validate_execution_marker(
        out_dir=out_dir,
        preflight_lock=lock_path,
        jobs=1,
        revalidate_operations=False,
...
def test_terminal_result_is_immutable_and_blocks_resume(
    ...
    terminal = _write_payload(
        out_dir / v4.TERMINAL_RESULT_NAME,
        {"version": v4.VERSION, "decision": "HOLD"},
    )
    terminal_hash = sha256_path(out_dir / v4.TERMINAL_RESULT_NAME)
    ...
    assert sha256_path(out_dir / v4.TERMINAL_RESULT_NAME) == terminal_hash
...
def test_checkpoint_resume_requires_same_marker(
    ...
    model, loaded = v4._load_checkpoint(
        out_dir=out_dir,
...
def test_state_machine_is_explicit_and_does_not_patch_core_marker() -> None:
    ...
    pipeline = inspect.getsource(v4._execute_pipeline)
...
def test_v3_hold_directory_remains_zero_work() -> None:
    audit = preflight._v3_zero_work_audit()
    assert audit["passes"], audit


def test_live_dashboard_rewrite_preserves_inventory_when_collision_free(
    tmp_path: Path,
) -> None:
    scan_root, live, internal, tasks, manifest = _collision_fixture(
        tmp_path
    )
    _write_history(live[0], logical_seed=21, deck_stream_id=22)
    _write_history(live[1], slot_stream_id=23, policy_stream_id=24)
    audit = v4.revalidate_collision_source_manifest(
        manifest,
        tasks,
        excluded_directories=internal,
        live_source_paths=live,
    ...
    assert audit["passes"], audit
    assert audit["checks"]["immutable_inventory_exact"]
    assert audit["checks"]["zero_collisions"]


def test_immutable_mutation_fails_closed(
    tmp_path: Path,
) -> None:
    scan_root, live, internal, tasks, manifest = _collision_fixture(
        tmp_path
    ...
    immutable = scan_root / "historical" / "manifest.json"
    _write_history(immutable, slot_stream_id=13, policy_stream_id=14)
    audit = v4.revalidate_collision_source_manifest(
        manifest,
        tasks,
        excluded_directories=internal,
    ...
    assert not audit["checks"]["immutable_inventory_exact"]
    assert audit["checks"]["zero_collisions"]


def test_new_unclassified_source_fails_closed(
    tmp_path: Path,
) -> None:
    scan_root, live, internal, tasks, manifest = _collision_fixture(
        tmp_path
    ...
    assert not audit["passes"]
    assert not audit["checks"]["immutable_source_count_exact"]
    assert not audit["checks"]["immutable_inventory_exact"]


def test_actual_collision_in_live_source_fails_closed(
    tmp_path: Path,
) -> None:
    scan_root, live, internal, tasks, manifest = _collision_fixture(
        tmp_path
    ...
    _write_history(
        live[0],
        deck_stream_id=tasks[10]["deck_stream_id"],
    )
    audit = v4.revalidate_collision_source_manifest(
        manifest,
        tasks,
    ...
    assert not audit["passes"]
    assert not audit["checks"]["zero_collisions"]
    assert audit["collisions"]["deck_stream_id"] == [
        tasks[10]["deck_stream_id"]
    ]


def test_missing_or_symlinked_live_source_fails_closed(
    tmp_path: Path,
) -> None:
    scan_root, live, internal, tasks, manifest = _collision_fixture(
        tmp_path
    ...
    live[0].unlink()
    missing = v4.revalidate_collision_source_manifest(
        manifest,
        tasks,
        excluded_directories=internal,
        live_source_paths=live,
    ...
    assert not missing["checks"]["live_paths_exact"]
    target = scan_root / "dashboard" / "replacement.json"
    _write_history(target, logical_seed=11)
    live[0].symlink_to(target)
    symlinked = v4.revalidate_collision_source_manifest(
        manifest,
        tasks,
        excluded_directories=internal,
        live_source_paths=live,
    ...
    assert not symlinked["passes"]
    assert not symlinked["checks"]["no_symlink_sources"]


def test_collision_manifest_binds_requested_stream_set(
    tmp_path: Path,
) -> None:
    scan_root, live, internal, tasks, manifest = _collision_fixture(
        tmp_path
    )
    changed = [dict(task) for task in tasks]
    changed[-1]["policy_stream_id"] += 1
    audit = v4.revalidate_collision_source_manifest(
        manifest,
        changed,
        excluded_directories=internal,
    ...
    assert not audit["passes"]
    assert not audit["checks"]["requested_stream_contract_exact"]
